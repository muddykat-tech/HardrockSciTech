...
public class IGBlockStateProvider extends BlockStateProvider {
    ...
    protected Logger logger = HRSTLib.getNewLogger();

    public IGBlockStateProvider(DataGenerator generator, ExistingFileHelper helper)
    {
        super(generator.getPackOutput(), HRSTLib.MODID, helper);
    ...
    {
        HRSTLib.HRST_LOGGER.info("Generating ["+ registry_name +"] Multiblock Model Data");
        createMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), HRSTRegistrationHolder.getMBTemplate.apply(registry_name));
    }

    private void genericmultiblockMirror(String registry_name)
    {
        HRSTLib.HRST_LOGGER.info("Generating ["+ registry_name +"] with Custom Mirror Multiblock Model Data");
        testCreateMultiblock(innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+".obj"), innerObj("block/multiblock/obj/"+registry_name+"/"+registry_name+"_mirrored.obj"),  (HRSTTemplateMultiblock) HRSTRegistrationHolder.getMBTemplate.apply(registry_name), false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, TemplateMultiblock multiblock)
    {
        createMultiblock(unsplitModel, (HRSTTemplateMultiblock) multiblock, false);
    }

    private void createMultiblock(NongeneratedModel unsplitModel, HRSTTemplateMultiblock multiblock, boolean dynamic)
    {
    ...
    private void testCreateMultiblock(NongeneratedModel unsplitModel, NongeneratedModel mirror_model, HRSTTemplateMultiblock multiblock, boolean dynamic)
    {
        final ModelFile mainModel = split(unsplitModel, multiblock, false, dynamic);
    ...
    private ModelFile split(NongeneratedModel loc, HRSTTemplateMultiblock mb)
    {
        return split(loc, mb, false);
    }

    private ModelFile split(NongeneratedModel loc, HRSTTemplateMultiblock mb, boolean mirror)
    {
        return split(loc, mb, mirror, false);
    }

    private ModelFile split(NongeneratedModel loc, HRSTTemplateMultiblock mb, boolean mirror, boolean dynamic)
    {
    ...
    private ModelFile split(
            NongeneratedModel name, HRSTTemplateMultiblock multiblock, UnaryOperator<BlockPos> transform, boolean dynamic
    )
    {
